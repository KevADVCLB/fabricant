...
void main() {
    ...
    vec3 col = texture2D(_Image, vUv + texelSize * _StepSize*_Scale).xyz;
    col += texture2D(_Image, vUv - texelSize * _StepSize*_Scale).xyz;
    col += texture2D(_Image, vUv + vec2(texelSize.x, -texelSize.y) * _StepSize*_Scale).xyz;
    col += texture2D(_Image, vUv + vec2(-texelSize.x, +texelSize.y) * _StepSize*_Scale).xyz;
    col /= 4.0;
